...
export default function CreateNewBoard() {
  ...
  const {dispatch, username, Boards, modals, currentBoard, currentBoardCopy, errorMessage } = useAuth()
  ...
  const [isOpen, setIsOpen] = useState(false);
    ...
    return (
        ...
        <div ref={modalRef} className='rounded-[10px] bg-[#2B2C37] w-full max-w-[30rem] min-w-[350px] p-8 fixed top-0 md:top-[10%] z-[99999] h-full md:min-h-[250px] md:max-h-[550px] md:flex md:flex-col '>  
            ...
            <h3 className='mb-4 text-[1.125rem] font-semibold text-white'>Add New Board</h3>
            <form onSubmit={newBoardFunction}>
                <div className='flex flex-col'>
                    ...
                    <label className='mb-2 flex justify-between items-center'>
                        ...
                        className={`w-full bg-transparent border-[2px] rounded-md px-4 py-2 text-[0.8125rem] font-semibold text-white transition-colors delay-200 ease-linear outline-none focus:border-[#635FC7] border-[#828ca366]  ${ errorMessage !=='' ? 'border-red-400' : 'border-[#828ca366]'}`}
                    ...
                    {currentBoardCopy.columns?.map(item=>{
                        return (
                            <label key={item.id} className='mb-2 flex justify-between items-center'>
                                <input 
                                type='text'
                                className={`${currentBoardCopy.columns?.length > 1 ? 'w-[90%]' : 'w-full'} bg-transparent border-[2px] rounded-md px-4 py-2 text-[0.8125rem] font-semibold text-white transition-colors delay-200 ease-linear outline-none focus:border-[#635FC7] border-[#828ca366]  ${ errorMessage !=='' ? 'border-red-400' : 'border-[#828ca366]'}`}
                                value={item.name}
                                onChange={(e)=>{
                                    dispatch({
                                        type: 'setError',
